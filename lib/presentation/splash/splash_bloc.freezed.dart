// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'splash_bloc.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

/// @nodoc
mixin _$SplashState {
  StateType get stateType => throw _privateConstructorUsedError;
  bool get navigateToOnBoarding => throw _privateConstructorUsedError;
  bool get navigateToLogin => throw _privateConstructorUsedError;
  bool get navigateToHome => throw _privateConstructorUsedError;
  bool get isForceUpdateEnabled => throw _privateConstructorUsedError;
  bool get isUnderMaintenanceEnabled => throw _privateConstructorUsedError;
  PlatformModel? get platformResponseModel =>
      throw _privateConstructorUsedError;

  @JsonKey(ignore: true)
  $SplashStateCopyWith<SplashState> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $SplashStateCopyWith<$Res> {
  factory $SplashStateCopyWith(
          SplashState value, $Res Function(SplashState) then) =
      _$SplashStateCopyWithImpl<$Res, SplashState>;
  @useResult
  $Res call(
      {StateType stateType,
      bool navigateToOnBoarding,
      bool navigateToLogin,
      bool navigateToHome,
      bool isForceUpdateEnabled,
      bool isUnderMaintenanceEnabled,
      PlatformModel? platformResponseModel});

  $PlatformModelCopyWith<$Res>? get platformResponseModel;
}

/// @nodoc
class _$SplashStateCopyWithImpl<$Res, $Val extends SplashState>
    implements $SplashStateCopyWith<$Res> {
  _$SplashStateCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? stateType = null,
    Object? navigateToOnBoarding = null,
    Object? navigateToLogin = null,
    Object? navigateToHome = null,
    Object? isForceUpdateEnabled = null,
    Object? isUnderMaintenanceEnabled = null,
    Object? platformResponseModel = freezed,
  }) {
    return _then(_value.copyWith(
      stateType: null == stateType
          ? _value.stateType
          : stateType // ignore: cast_nullable_to_non_nullable
              as StateType,
      navigateToOnBoarding: null == navigateToOnBoarding
          ? _value.navigateToOnBoarding
          : navigateToOnBoarding // ignore: cast_nullable_to_non_nullable
              as bool,
      navigateToLogin: null == navigateToLogin
          ? _value.navigateToLogin
          : navigateToLogin // ignore: cast_nullable_to_non_nullable
              as bool,
      navigateToHome: null == navigateToHome
          ? _value.navigateToHome
          : navigateToHome // ignore: cast_nullable_to_non_nullable
              as bool,
      isForceUpdateEnabled: null == isForceUpdateEnabled
          ? _value.isForceUpdateEnabled
          : isForceUpdateEnabled // ignore: cast_nullable_to_non_nullable
              as bool,
      isUnderMaintenanceEnabled: null == isUnderMaintenanceEnabled
          ? _value.isUnderMaintenanceEnabled
          : isUnderMaintenanceEnabled // ignore: cast_nullable_to_non_nullable
              as bool,
      platformResponseModel: freezed == platformResponseModel
          ? _value.platformResponseModel
          : platformResponseModel // ignore: cast_nullable_to_non_nullable
              as PlatformModel?,
    ) as $Val);
  }

  @override
  @pragma('vm:prefer-inline')
  $PlatformModelCopyWith<$Res>? get platformResponseModel {
    if (_value.platformResponseModel == null) {
      return null;
    }

    return $PlatformModelCopyWith<$Res>(_value.platformResponseModel!, (value) {
      return _then(_value.copyWith(platformResponseModel: value) as $Val);
    });
  }
}

/// @nodoc
abstract class _$$_SplashStateCopyWith<$Res>
    implements $SplashStateCopyWith<$Res> {
  factory _$$_SplashStateCopyWith(
          _$_SplashState value, $Res Function(_$_SplashState) then) =
      __$$_SplashStateCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {StateType stateType,
      bool navigateToOnBoarding,
      bool navigateToLogin,
      bool navigateToHome,
      bool isForceUpdateEnabled,
      bool isUnderMaintenanceEnabled,
      PlatformModel? platformResponseModel});

  @override
  $PlatformModelCopyWith<$Res>? get platformResponseModel;
}

/// @nodoc
class __$$_SplashStateCopyWithImpl<$Res>
    extends _$SplashStateCopyWithImpl<$Res, _$_SplashState>
    implements _$$_SplashStateCopyWith<$Res> {
  __$$_SplashStateCopyWithImpl(
      _$_SplashState _value, $Res Function(_$_SplashState) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? stateType = null,
    Object? navigateToOnBoarding = null,
    Object? navigateToLogin = null,
    Object? navigateToHome = null,
    Object? isForceUpdateEnabled = null,
    Object? isUnderMaintenanceEnabled = null,
    Object? platformResponseModel = freezed,
  }) {
    return _then(_$_SplashState(
      stateType: null == stateType
          ? _value.stateType
          : stateType // ignore: cast_nullable_to_non_nullable
              as StateType,
      navigateToOnBoarding: null == navigateToOnBoarding
          ? _value.navigateToOnBoarding
          : navigateToOnBoarding // ignore: cast_nullable_to_non_nullable
              as bool,
      navigateToLogin: null == navigateToLogin
          ? _value.navigateToLogin
          : navigateToLogin // ignore: cast_nullable_to_non_nullable
              as bool,
      navigateToHome: null == navigateToHome
          ? _value.navigateToHome
          : navigateToHome // ignore: cast_nullable_to_non_nullable
              as bool,
      isForceUpdateEnabled: null == isForceUpdateEnabled
          ? _value.isForceUpdateEnabled
          : isForceUpdateEnabled // ignore: cast_nullable_to_non_nullable
              as bool,
      isUnderMaintenanceEnabled: null == isUnderMaintenanceEnabled
          ? _value.isUnderMaintenanceEnabled
          : isUnderMaintenanceEnabled // ignore: cast_nullable_to_non_nullable
              as bool,
      platformResponseModel: freezed == platformResponseModel
          ? _value.platformResponseModel
          : platformResponseModel // ignore: cast_nullable_to_non_nullable
              as PlatformModel?,
    ));
  }
}

/// @nodoc

class _$_SplashState with DiagnosticableTreeMixin implements _SplashState {
  const _$_SplashState(
      {this.stateType = StateType.success,
      this.navigateToOnBoarding = false,
      this.navigateToLogin = false,
      this.navigateToHome = false,
      this.isForceUpdateEnabled = false,
      this.isUnderMaintenanceEnabled = false,
      this.platformResponseModel});

  @override
  @JsonKey()
  final StateType stateType;
  @override
  @JsonKey()
  final bool navigateToOnBoarding;
  @override
  @JsonKey()
  final bool navigateToLogin;
  @override
  @JsonKey()
  final bool navigateToHome;
  @override
  @JsonKey()
  final bool isForceUpdateEnabled;
  @override
  @JsonKey()
  final bool isUnderMaintenanceEnabled;
  @override
  final PlatformModel? platformResponseModel;

  @override
  String toString({DiagnosticLevel minLevel = DiagnosticLevel.info}) {
    return 'SplashState(stateType: $stateType, navigateToOnBoarding: $navigateToOnBoarding, navigateToLogin: $navigateToLogin, navigateToHome: $navigateToHome, isForceUpdateEnabled: $isForceUpdateEnabled, isUnderMaintenanceEnabled: $isUnderMaintenanceEnabled, platformResponseModel: $platformResponseModel)';
  }

  @override
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties
      ..add(DiagnosticsProperty('type', 'SplashState'))
      ..add(DiagnosticsProperty('stateType', stateType))
      ..add(DiagnosticsProperty('navigateToOnBoarding', navigateToOnBoarding))
      ..add(DiagnosticsProperty('navigateToLogin', navigateToLogin))
      ..add(DiagnosticsProperty('navigateToHome', navigateToHome))
      ..add(DiagnosticsProperty('isForceUpdateEnabled', isForceUpdateEnabled))
      ..add(DiagnosticsProperty(
          'isUnderMaintenanceEnabled', isUnderMaintenanceEnabled))
      ..add(
          DiagnosticsProperty('platformResponseModel', platformResponseModel));
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$_SplashState &&
            (identical(other.stateType, stateType) ||
                other.stateType == stateType) &&
            (identical(other.navigateToOnBoarding, navigateToOnBoarding) ||
                other.navigateToOnBoarding == navigateToOnBoarding) &&
            (identical(other.navigateToLogin, navigateToLogin) ||
                other.navigateToLogin == navigateToLogin) &&
            (identical(other.navigateToHome, navigateToHome) ||
                other.navigateToHome == navigateToHome) &&
            (identical(other.isForceUpdateEnabled, isForceUpdateEnabled) ||
                other.isForceUpdateEnabled == isForceUpdateEnabled) &&
            (identical(other.isUnderMaintenanceEnabled,
                    isUnderMaintenanceEnabled) ||
                other.isUnderMaintenanceEnabled == isUnderMaintenanceEnabled) &&
            (identical(other.platformResponseModel, platformResponseModel) ||
                other.platformResponseModel == platformResponseModel));
  }

  @override
  int get hashCode => Object.hash(
      runtimeType,
      stateType,
      navigateToOnBoarding,
      navigateToLogin,
      navigateToHome,
      isForceUpdateEnabled,
      isUnderMaintenanceEnabled,
      platformResponseModel);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$_SplashStateCopyWith<_$_SplashState> get copyWith =>
      __$$_SplashStateCopyWithImpl<_$_SplashState>(this, _$identity);
}

abstract class _SplashState implements SplashState {
  const factory _SplashState(
      {final StateType stateType,
      final bool navigateToOnBoarding,
      final bool navigateToLogin,
      final bool navigateToHome,
      final bool isForceUpdateEnabled,
      final bool isUnderMaintenanceEnabled,
      final PlatformModel? platformResponseModel}) = _$_SplashState;

  @override
  StateType get stateType;
  @override
  bool get navigateToOnBoarding;
  @override
  bool get navigateToLogin;
  @override
  bool get navigateToHome;
  @override
  bool get isForceUpdateEnabled;
  @override
  bool get isUnderMaintenanceEnabled;
  @override
  PlatformModel? get platformResponseModel;
  @override
  @JsonKey(ignore: true)
  _$$_SplashStateCopyWith<_$_SplashState> get copyWith =>
      throw _privateConstructorUsedError;
}
