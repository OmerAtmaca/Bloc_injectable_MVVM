...
const Curve _kResizeTimeCurve = Curves.easeInOut;
const double _kMinFlingVelocity = 700.0;
const double _kMinFlingVelocityDelta = 400.0;
const double _kFlingVelocityScale = 1.0 / 300.0;
const double _kDismissThreshold = 0.4;

/// Signature used by [DismissibleStack] to indicate that it has been dismissed in
/// the given `direction`.
///
/// Used by [DismissibleStack.onDismissed].
typedef DismissDirectionCallback = void Function(DismissDirection direction);

/// Signature used by [DismissibleStack] to give the application an opportunity to
/// confirm or veto a dismiss gesture.
///
/// Used by [DismissibleStack.confirmDismiss].
typedef ConfirmDismissCallback = Future<bool?> Function(
    DismissDirection direction);

/// Signature used by [DismissibleStack] to indicate that the dismissibleStack has been dragged.
///
/// Used by [DismissibleStack.onUpdate].
typedef DismissUpdateCallback = void Function(DismissUpdateDetails details);

/// The direction in which a [DismissibleStack] can be dismissed.
enum DismissDirection {
  /// The [DismissibleStack] can be dismissed by dragging either up or down.
  vertical,

  /// The [DismissibleStack] can be dismissed by dragging either left or right.
  horizontal,

  /// The [DismissibleStack] can be dismissed by dragging in the reverse of the
  /// reading direction (e.g., from right to left in left-to-right languages).
  endToStart,

  /// The [DismissibleStack] can be dismissed by dragging in the reading direction
  /// (e.g., from left to right in left-to-right languages).
  startToEnd,

  /// The [DismissibleStack] can be dismissed by dragging up only.
  up,

  /// The [DismissibleStack] can be dismissed by dragging down only.
  down,

  /// The [DismissibleStack] cannot be dismissed by dragging.
  none
}
...
/// non-null, the DismissibleStack widget animates its height (or width, whichever is
...
///
/// {@tool dartpad}
/// This sample shows how you can use the [DismissibleStack] widget to
/// remove list items using swipe gestures. Swipe any of the list
/// tiles to the left or right to dismiss them from the [ListView].
///
/// ** See code in examples/api/lib/widgets/dismissibleStack/dismissibleStack.0.dart **
/// {@end-tool}
///
/// Backgrounds can be used to implement the "leave-behind" idiom. If a background
/// is specified it is stacked behind the DismissibleStack's child and is exposed when
/// the child moves.
///
/// The widget calls the [onDismissed] callback either after its size has
/// collapsed to zero (if [resizeDuration] is non-null) or immediately after
/// the slide animation (if [resizeDuration] is null). If the DismissibleStack is a
/// list item, it must have a key that distinguishes it from the other items and
/// its [onDismissed] callback must remove the item from the list.
class DismissibleStack extends StatefulWidget {
  /// Creates a widget that can be dismissed.
  ///
  /// The [key] argument must not be null because [DismissibleStack]s are commonly
  /// used in lists and removed from the list when dismissed. Without keys, the
  /// default behavior is to sync widgets based on their index in the list,
  /// which means the item after the dismissed item would be synced with the
  /// state of the dismissed item. Using keys causes the widgets to sync
  /// according to their keys and avoids this pitfall.
  const DismissibleStack({
    required Key key,
  ...
  final VoidCallback? onTopCardDismissed;
  ...
  /// dismissibleStack will begin at the position where the drag gesture won the arena.
  ...
  /// Called when the dismissibleStack widget has been dragged.
  ///
...
/// See also:
///
///   * [DismissibleStack.onUpdate], which receives this information.
class DismissUpdateDetails {
  ...
  /// The direction that the dismissibleStack is being dragged.
  final DismissDirection direction;
  ...
  /// that the [DismissibleStack] is dragged across the threshold.
  final bool previousReached;

  /// The offset ratio of the dismissibleStack in its parent container.
  ///
  /// A value of 0.0 represents the normal position and 1.0 means the child is
  /// completely outside its parent.
  ///
  /// This can be used to synchronize other elements to what the dismissibleStack is doing on screen,
  /// e.g. using this value to set the opacity thereby fading dismissibleStack as it's dragged offscreen.
  final double progress;
}

class _DismissibleStackClipper extends CustomClipper<Rect> {
  _DismissibleStackClipper({
    required this.axis,
  ...
  @override
  bool shouldReclip(_DismissibleStackClipper oldClipper) {
...
class _DismissibleStackState extends State<DismissibleStack>
  ...
  AnimationController? _topCardController;
  Animation<double>? _topCardAnimation;
  ...
  bool get wantKeepAlive =>
      ...
      (_topCardController?.isAnimating ?? false) ||
  ...
  void _handleDragStart(DragStartDetails details) {
    if (_topCardController?.isAnimating ?? false) {
      return;
    }
    if (_confirming) {
      return;
  ...
  Future<void> _handleMoveCompleted() async {
    ...
    if (mounted) {
      if (result) {
        final isLastChild = widget.children.length == 1;

        if (isLastChild) {
          _startResizeAnimation();
        } else {
          _startTopCardAnimation();
        }
      } else {
        _moveController!.reverse();
      }
  ...
  void _startTopCardAnimation() {
    _topCardController =
        AnimationController(duration: widget.resizeDuration, vsync: this)
          ..addListener(_handleTopCardProgressChanged)
          ..addStatusListener((AnimationStatus status) => updateKeepAlive());
    _topCardController!.forward();
    setState(() {
      _topCardAnimation = _topCardController!
          .drive(
            CurveTween(
              curve: _kResizeTimeCurve,
            ),
  ...
  void _handleTopCardProgressChanged() {
    if (_topCardAnimation!.isCompleted) {
      if (widget.onTopCardDismissed != null) {
        widget.onTopCardDismissed!();
      }
      _moveController?.reset();
      setState(() {
        _topCardAnimation = null;
      });
    }
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
      ...
      assert(() {
        if (_resizeAnimation!.status != AnimationStatus.forward) {
          ...
          throw FlutterError.fromParts(<DiagnosticsNode>[
            ErrorSummary(
                'A dismissed DismissibleStack widget is still part of the tree.'),
            ErrorHint(
              'Make sure to implement the onDismissed handler and to immediately remove the DismissibleStack '
    ...
    final stepHeight = widget.childHeight * 0.1;

    Widget content = Stack(children: <Widget>[
      SizedBox(height: widget.childHeight + stepHeight),
      if (!_moveAnimation.isDismissed)
        Positioned.fill(
          child: ClipRect(
            clipper: _DismissibleStackClipper(
      ...
      if (widget.children.length - 4 >= 0 && _topCardAnimation != null)
        Transform.translate(
          offset: getTopCardAnimatinValue<Offset>(
              begin: Offset(0, stepHeight * 3), end: Offset(0, stepHeight * 2)),
          child: Transform.scale(
            scale: getTopCardAnimatinValue<double>(begin: 0.7, end: 0.8),
            child: FadeTransition(
              opacity: _topCardAnimation!,
              child: widget.children[widget.children.length - 4],
            ),
          ),
        ),
      if (widget.children.length - 3 >= 0)
        Transform.translate(
          offset: getTopCardAnimatinValue<Offset>(
              begin: Offset(0, stepHeight * 2), end: Offset(0, stepHeight)),
          child: Transform.scale(
            scale: getTopCardAnimatinValue<double>(begin: 0.8, end: 0.9),
            child: widget.children[widget.children.length - 3],
          ),
        ),
      if (widget.children.length - 2 >= 0)
        Transform.translate(
          offset: getTopCardAnimatinValue<Offset>(
              begin: Offset(0, stepHeight), end: Offset.zero),
          child: Transform.scale(
            scale: getTopCardAnimatinValue<double>(begin: 0.9, end: 1),
            child: widget.children[widget.children.length - 2],
          ),
        ),
      if (widget.children.isNotEmpty)
        SlideTransition(
          position: _moveAnimation,
          child: KeyedSubtree(key: _contentKey, child: widget.children.last),
        ),
    ]);
  ...
  T getTopCardAnimatinValue<T>({required T begin, required T end}) {
    return _topCardAnimation == null
        ? begin
        : Tween<T>(begin: begin, end: end).animate(_topCardAnimation!).value;
  }
}
